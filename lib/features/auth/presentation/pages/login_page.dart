...
class _LoginPageState extends State<_LoginPageContent> {
    ...
    return BlocListener<AuthBloc, AuthState>(
      listener: (context, state) {
        if (state is AuthLoading) {
          ...
          // Save user session using AppRouter
          final userId = state.user.email;
          final userToken =
              state.user.thirdPartyToken ??
              'default_token_${DateTime.now().millisecondsSinceEpoch}';

          AppRouter.loginUser(context, userId: userId, userToken: userToken);
  ...
  void _handleLogin() {
    ...
    // For demo purposes, allow any email/password for testing zones
    if (_emailController.text.contains('@') &&
        _passwordController.text.length >= 3) {
      print('🎯 UI: Demo login successful');

      // Demo user session
      final userId = _emailController.text.trim();
      final userToken = 'demo_token_${DateTime.now().millisecondsSinceEpoch}';

      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Welcome to Ru\'yaAI Demo!'),
          backgroundColor: Colors.green,
    ...
    // Original BLoC-based authentication for production
